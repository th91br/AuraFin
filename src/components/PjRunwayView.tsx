  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200 text-slate-100">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div>
          <h1 className="text-2xl font-black text-white">Runway &amp; Sobrevivência de Caixa</h1>
          <p className="text-xs text-slate-300 mt-1">Estimativa de tempo em que a operação se mantém com o caixa atual e ritmo de queima (burn rate).</p>
        </div>
      ...
      {!hasData ? (
        <div className="p-16 text-center bg-slate-900/80 rounded-2xl border border-dashed border-white/10 text-slate-300">
          Nenhum dado de saldo ou movimentações disponível para simulação de runway.
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <MetricCard title="Caixa Disponível" value={cash} isPrivacyMode={isPrivacyMode} subtitle="Contas bancárias PJ" />
            <MetricCard title="Burn Rate Mensal" value={burn} isPrivacyMode={isPrivacyMode} subtitle="Despesas operacionais" />
            <MetricCard title="Runway em Meses" value={Number(months.toFixed(1))} prefix="" subtitle="Meses de operação" />
            <MetricCard title="Runway em Dias" value={days} prefix="" subtitle="Dias de cobertura" />
          </div>
